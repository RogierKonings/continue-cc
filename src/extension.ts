...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const signOutCommand = vscode.commands.registerCommand('claude-code-continue.signOut', async () => {
    await authFlowCoordinator.startLogoutFlow();
    await authStatusBar.updateStatus();
  });

  // Register auth menu command
  const authMenuCommand = vscode.commands.registerCommand('claude-code-continue.showAuthMenu', async () => {
  ...
  const welcomeViewProvider = vscode.window.createTreeDataProvider('claude-code-continue.welcome', {
